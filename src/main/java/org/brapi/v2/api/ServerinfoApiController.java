...
public class ServerinfoApiController implements ServerinfoApi {
    ...
    public ResponseEntity<CallsResponse> serverinfoGet(@ApiParam(value = "The data format supported by the call.") @Valid @RequestParam(value = "dataType", required = false) WSMIMEDataTypes dataType,@ApiParam(value = "HTTP HEADER - Token used for Authorization   <strong> Bearer {token_string} </strong>" ) @RequestHeader(value="Authorization", required=false) String authorization) {
		...
		result.addCallsItem(new Service() {{ getVersions().add(VersionsEnum._1); getMethods().addAll(Arrays.asList(MethodsEnum.GET,MethodsEnum.PUT)); setService(CallsApi.callsGet_url); setDataTypes(new ArrayList<WSMIMEDataTypes>() {{ add(WSMIMEDataTypes.fromValue("application/json")); }}); }});
		...
		result.addCallsItem(new Service() {{ getVersions().add(VersionsEnum._1); getMethods().add(MethodsEnum.GET); setService(AllelematrixApi.alleleMatrixGet_url); setDataTypes(new ArrayList<WSMIMEDataTypes>() {{ add(WSMIMEDataTypes.fromValue("application/json")); }}); }});
		...
		result.addCallsItem(new Service() {{ getVersions().add(VersionsEnum._1); getMethods().add(MethodsEnum.POST); setService(GermplasmApi.searchGermplasmPost_url); setDataTypes(new ArrayList<WSMIMEDataTypes>() {{ add(WSMIMEDataTypes.fromValue("application/json")); }}); }});
		...
		result.addCallsItem(new Service() {{ getVersions().add(VersionsEnum._0); getMethods().add(MethodsEnum.POST); setService(AttributevaluesApi.searchAttributevaluesPost_url); setDataTypes(new ArrayList<WSMIMEDataTypes>() {{ add(WSMIMEDataTypes.fromValue("application/json")); }}); }});
                result.addCallsItem(new Service() {{ getVersions().add(VersionsEnum._1); getMethods().add(MethodsEnum.POST); setService(AllelematrixApi.searchAlleleMatrixPost_url); setDataTypes(new ArrayList<WSMIMEDataTypes>() {{ add(WSMIMEDataTypes.fromValue("application/json")); }}); }});
