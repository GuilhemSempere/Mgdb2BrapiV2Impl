...
//    }
    ...
    @Override
	public ResponseEntity<CallListResponse2> callsGet2(String callSetDbId, String variantDbId, String variantSetDbId, Boolean expandHomozygotes, String unknownString, String sepPhased, String sepUnphased, String pageToken, Integer pageSize, String authorization) throws SocketException, UnknownHostException, UnsupportedEncodingException {
		if (variantSetDbId == null && callSetDbId != null) {
			String[] info = GigwaSearchVariantsRequest.getInfoFromId(callSetDbId, 3);
			GenotypingSample sample = MongoTemplateManager.get(info[0]).find(new Query(Criteria.where("_id").is(Integer.parseInt(info[2]))), GenotypingSample.class).iterator().next();
			variantSetDbId = info[0] + IGigwaService.ID_SEPARATOR + sample.getProjectId() + IGigwaService.ID_SEPARATOR + sample.getRun();
		...
		return searchCallsPost2(csr, authorization);
	}
	
	@Override
	public ResponseEntity<CallListResponse2> searchCallsPost2(CallsSearchRequest body, String authorization) throws SocketException, UnknownHostException, UnsupportedEncodingException {
		String token = ServerinfoApiController.readToken(authorization);

    	CallListResponse2 clr = new CallListResponse2();
    	CallsListResponseResult2 result = new CallsListResponseResult2();
    	MetadataTokenPagination metadata = new MetadataTokenPagination();
		clr.setMetadata(metadata);
...
//							genotype = StringUtils.join(alleles, fPhased ? phasedSeparator : unPhasedSeparator);
        			...
        			ListValue lv = new ListValue();
        			call.setGenotype(gtCode);
        			call.setVariantDbId(module + GigwaGa4ghServiceImpl.ID_SEPARATOR + vrd.getId().getVariantId());
        			call.setVariantName(call.getVariantDbId());
        			call.setCallSetDbId(module + GigwaGa4ghServiceImpl.ID_SEPARATOR + sampleIndividuals.get(spId) + GigwaGa4ghServiceImpl.ID_SEPARATOR + spId);
        			call.setCallSetName(call.getCallSetDbId());
                                call.setRefAllele(vrd.getKnownAlleles().get(0));
                                call.setAltAllele(vrd.getKnownAlleles().get(1));
        			...
        			for (String key : sg.getAdditionalInfo().keySet()) {
                                    GenotypeField gf = new GenotypeField();
                                    gf.setFieldAbbreviation(key);
                                    gf.setFieldvalue(sg.getAdditionalInfo().get(key).toString());
                                    call.addGenotypeField(gf);
                                }
			...
			clr.setResult(result);
			return new ResponseEntity<CallListResponse2>(clr, HttpStatus.OK);
        } catch (Exception e) {
            log.error("Couldn't serialize response for content type application/json", e);
            return new ResponseEntity<CallListResponse2>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
	}

    @Override
    public ResponseEntity<CallListResponse2> callsPut(CallsListResponseResult2 body, String authorization) throws SocketException, UnknownHostException, UnsupportedEncodingException {
        String token = ServerinfoApiController.readToken(authorization);
        
        CallListResponse2 response = new CallListResponse2();
        
        List<Call2> callsToUpdate =  body.getData();
        String module = null;
        for (Call2 c:callsToUpdate) {
            if (c.getVariantSetDbId() != null) {
                ...
                Status status = new Status();
                status.setMessage("You must specify VariantSets belonging to the same referenceSet!");
                response.setMetadata(new Metadata().addStatusItem(status));
        ...
        List<Call2> updatedCalls = new ArrayList<>();
        
        for (Call2 c:callsToUpdate) {
            
            if (c.getCallSetDbId().equals("snpclus§P22§22")) {
                String gen = c.getGenotype();
            }
            ...
            if (c.getVariantDbId() == null || c.getVariantSetDbId() == null || c.getCallSetDbId() == null) {
               return new ResponseEntity<>(HttpStatus.FORBIDDEN);
                ...
                String phasedSeparator = body.getSepPhased() == null ? "|" : URLDecoder.decode(body.getSepPhased(), "UTF-8");
                String unPhasedSeparator = body.getSepUnphased() == null ? "/" : body.getSepUnphased();
                //response.getResult().setSepUnphased(unPhasedSeparator);

                Update update = new Update();
                update.set(VariantRunData.FIELDNAME_SAMPLEGENOTYPES + "." + splitCallSetDbId[2] + "." + SampleGenotype.FIELDNAME_GENOTYPECODE, c.getGenotype());
