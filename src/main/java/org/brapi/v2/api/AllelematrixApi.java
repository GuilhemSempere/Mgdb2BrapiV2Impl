...
public interface AllelematrixApi {
    ...
    ResponseEntity<AlleleMatrixResponse> allelematrixGet(@Parameter(in = ParameterIn.QUERY, description = "The requested page number for the Variant dimension of the matrix", schema = @Schema()) @Valid @RequestParam(value = "dimensionVariantPage", required = false) Integer dimensionVariantPage,
            ...
            @Parameter(in = ParameterIn.QUERY, description = "The requested page number for the column (callset, sample or germplasm) dimension of the matrix", schema = @Schema()) @Valid @RequestParam(value = "dimensionColumnPage", required = false) Integer dimensionColumnPage,
            @Parameter(in = ParameterIn.QUERY, description = "The requested page size for the column (callset, sample or germplasm) dimension of the matrix", schema = @Schema()) @Valid @RequestParam(value = "dimensionColumnPageSize", required = false) Integer dimensionColumnPageSize,
            ...
            @Parameter(in = ParameterIn.QUERY, description = "The unique database identifier of a Study. Filters both the row dimension (via the VariantSets associated with the Study) and the column dimension (via the Samples associated with those VariantSets). Acts as an AND constraint alongside all other filters.", schema = @Schema()) @Valid @RequestParam(value = "studyDbId", required = false) String studyDbId,
            @Parameter(in = ParameterIn.QUERY, description = "Override the default column aggregation granularity. Accepted values: 'CALLSET' (default, one column per CallSet), 'SAMPLE' (one column per Sample, genotypes aggregated by majority vote), 'GERMPLASM' (one column per Germplasm, genotypes aggregated by majority vote). When a tie occurs, the column is treated as missing data.", schema = @Schema(allowableValues = {"CALLSET", "SAMPLE", "GERMPLASM"})) @Valid @RequestParam(value = "dimensionColumnAggregation", required = false) AlleleMatrixSearchRequest.DimensionColumnAggregationEnum dimensionColumnAggregation,
            @Parameter(in = ParameterIn.HEADER, description = "HTTP HEADER - Token used for Authorization   <strong> Bearer {token_string} </strong>", schema = @Schema()) @RequestHeader(value = "Authorization", required = false) String authorization) throws InterruptedException, ObjectNotFoundException;
    ...
    ResponseEntity<AlleleMatrixResponse> searchAllelematrixPost(
            ...
            @Parameter(in = ParameterIn.DEFAULT, description = "Study Search request", schema=@Schema()) @Valid @RequestBody AlleleMatrixSearchRequest body) throws InterruptedException, ObjectNotFoundException;
